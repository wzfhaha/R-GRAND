...
from model import BaseGCN, SamplingGCN
...
def evaluate(model, valid_dataloader, features, labels, use_embed, loss_func, device):
    model.eval()
    ys = []
    logits_list = []
    for it, (input_nodes, output_nodes, blocks) in enumerate(valid_dataloader):
        with torch.no_grad():
            ...
            if use_embed:
                ...
                feat = features[input_nodes].to(device)
            logits = model(blocks, feat, device)
            logits_list.append(logits)
            ys.append(labels[output_nodes])
    y = torch.cat(ys)
    logits = torch.cat(logits_list)
    loss = loss_func(logits, y)
    pred_result = logits.argmax(dim=1)
    y = y.cpu()
    pred_result = pred_result.cpu()
    micro = f1_score(y, pred_result, average='micro')
    macro = f1_score(y, pred_result, average='macro')

    return loss, micro, macro
    #loss = LOSS(logits, labels[output_nodes])
"""
...
def main(args):
    ...
    features_list = [mat2tensor(features, args.feat_norm)
    ...
    # num_classes = labels.max() + 1
    print(num_classes)
    use_embed = args.dataset == 'Freebase'
    model = SamplingGCN(in_dim,
                           args.hidden_dim,
                           num_classes,
                           device,
    ...
    train_dataloader = dgl.dataloading.DataLoader(
        g, train_idx, sampler, batch_size=1024, shuffle=True, drop_last=False, num_workers=10
    )
    valid_dataloader = dgl.dataloading.DataLoader(
        g, val_idx, sampler, batch_size=1024, shuffle=True, drop_last=False, num_workers=10
    )
    full_sampler = dgl.dataloading.MultiLayerFullNeighborSampler(3)
    test_inf_dataloader = dgl.dataloading.DataLoader(
        g, test_idx, full_sampler, batch_size=10, shuffle=False, drop_last=False, num_workers=10
    )
    valid_inf_dataloader = dgl.dataloading.DataLoader(
        g, val_idx, full_sampler, batch_size=10, shuffle=False, drop_last=False, num_workers=10
    )
    # training loop
    ...
    # print(edge_type.max)
    # model.train()
    early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='checkpoint/checkpoint_{}_{}_{}_{}.pt'.format(args.dataset, args.num_layers, args.hidden_dim, args.train_num))
    batch_idx = 0
    features_all = None
    if not use_embed:
    ...
    for epoch in range(args.epoch):
        for it, (input_nodes, output_nodes, blocks) in enumerate(train_dataloader):      
            ...
            if use_embed:
                ...
                features = features_all[input_nodes].to(device)
            logits = model(blocks, features, device)
            # print('logits', logits)
            #logits = logits[train_idx]
            # ipdb.set_trace()
            # print(labels[output_nodes])
            loss = LOSS(logits, labels[output_nodes])
            t1 = time.time()
            loss.backward()
            optimizer.step()
            ...
            """
            if batch_idx%args.eval_batch_num == 0:
                val_loss, _, _ = evaluate(model, valid_dataloader, features_all, labels, use_embed, LOSS, device)
    ...
    model.load_state_dict(torch.load('checkpoint/checkpoint_{}_{}_{}_{}.pt'.format(args.dataset, args.num_layers, args.hidden_dim, args.train_num)))
    model.eval()
    ...
    with torch.no_grad():
        ...
        valid_loss, valid_micro, valid_macro = evaluate(model, valid_inf_dataloader, features_all, labels, use_embed, LOSS, device)
        test_loss, test_micro, test_macro = evaluate(model, test_inf_dataloader, features_all, labels, use_embed, LOSS, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_batch_num', type=int, default=1, help='valid num')
