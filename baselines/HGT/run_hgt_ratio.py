...
from model import BaseHGT
import json
from sklearn.metrics import f1_score
from scipy import sparse
# from dgl.nn.pytorch.conv import HGTConv
from model import HGTConv_s, HGTConv
import dgl
...
class EntityClassify(BaseHGT):
    def build_input_layer(self):
        return nn.ModuleList([nn.Linear(in_dim, self.h_dim * self.num_heads, bias=True) for in_dim in self.in_dims])

    def build_hidden_layer(self, idx):
        if self.simplify:
            return HGTConv_s(self.h_dim * self.num_heads, self.h_dim, self.num_heads, self.node_type, self.edge_type,
                            use_norm=self.layer_norm, activation=None, dropout=self.dropout, residue=self.residue)
        elif self.original:
            return HGTConv(self.h_dim * self.num_heads, self.h_dim, self.num_heads, self.node_type, self.edge_type,
                            use_norm=self.layer_norm, activation=None, dropout=self.dropout, residue=self.residue)
        else:
            return HGTConv(self.h_dim * self.num_heads, self.h_dim, self.num_heads, self.node_type, self.edge_type,
                            use_norm=self.layer_norm, activation=None, dropout=self.dropout, residue = self.residue)
    ...
    def build_output_layer(self):
        if self.simplify:
            return HGTConv_s(self.h_dim * self.num_heads, self.out_dim, self.num_heads, self.node_type, self.edge_type,
                            use_norm=False, activation=None, dropout=self.dropout)
        elif self.original:
            return HGTConv(self.h_dim * self.num_heads, self.out_dim, self.num_heads, self.node_type, self.edge_type,
                            use_norm=False, activation=None, dropout=self.dropout)
        else:
            return HGTConv(self.h_dim * self.num_heads, self.out_dim, self.num_heads, self.node_type, self.edge_type,
                            use_norm=False, activation=None, dropout=self.dropout)
    """
    def build_output_layer(self):
        return nn.Linear(self.h_dim * self.num_heads, self.out_dim)
def main(args):
    ...
    edge_type = e_feat
    node_type_num = len(num_nodes)
    # num_classes = labels#dl.labels_train['num_classes']
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), weight_decay=args.l2norm)
    #optimizer = torch.optim.AdamW(model.parameters(), weight_decay=args.weight_decay)#, lr=args.lr, weight_decay=args.weight_decay)
    # scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, total_steps=500, max_lr = args.lr, pct_start=0.05)
    # training loop
    ...
    early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='checkpoint/checkpoint_{}_{}_{}_{}.pt'.format(args.dataset, args.num_layers, args.hidden_dim, args.simplify))
    train_step = 0
    for epoch in range(args.epoch):
        optimizer.zero_grad()
        t0 = time.time()
        logits = model(g, features_list, node_type, edge_type)
        # print('logits', logits)
        ...
        if early_stopping.early_stop:
            ...
            break
    model.load_state_dict(torch.load('checkpoint/checkpoint_{}_{}_{}_{}.pt'.format(args.dataset, args.num_layers, args.hidden_dim, args.simplify)))
    model.eval()
    ...
    with torch.no_grad():
        ...
        t0 = time.time()
        logits = model.forward(g, features_list, node_type, edge_type)
        t1 = time.time()
    ...
    torch.cuda.empty_cache() 
    np.savez('loss.npz', train_loss=np.asarray(train_losses), test_micro=np.asarray(test_micros))
...
def opt_objective(trial, args):
    ...
    args.num_heads = trial.suggest_float('num_heads', 0, 50)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--residue', default=False, action='store_true', help='hyperparam search')
    parser.add_argument('--trials', type=int, default=30, help='hyperparam search')
    parser.add_argument('--schedule_step', type=int, default=300)
    ...
    if args.search:
        search_space = {'hidden_dim': [8,16], 'num_heads':[8,16], 'num_layers':[1,2,3], 'dropout':[0.3, 0.5]}
        study = optuna.create_study(sampler=optuna.samplers.GridSampler(search_space))
        study.optimize(lambda trial: opt_objective(trial, args), n_trials=args.trials)
        joblib.dump(study, f'{args.dataset}_{args.simplify}_study.pkl')
        run_best(args, study)
    else:
        valid_res_list = []
        test_res_list = []
