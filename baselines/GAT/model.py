...
class BaseGAT(nn.Module):
    def __init__(self, in_dims, h_dim, out_dim, num_heads,
    ...
    def build_model(self):
        ...
        if self.layer_norm:
            ...
            for idx in range(self.num_hidden_layers):
                self.norm_layers.append(nn.LayerNorm(self.h_dim * self.num_heads))
        # h2o
    ...
    def forward(self, g, features_list):
        ...
        for i, layer in enumerate(self.layers):
            h = layer(g, h)
            h = torch.reshape(h, (h.shape[0], self.num_heads * self.h_dim))
            if self.layer_norm:
