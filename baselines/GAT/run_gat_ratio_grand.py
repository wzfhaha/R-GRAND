...
from model import BaseGAT, GATConv
...
class EntityClassify(BaseGAT):
    def build_input_layer(self):
        return nn.ModuleList([nn.Linear(in_dim, self.h_dim * self.num_heads, bias=True) for in_dim in self.in_dims])

    def build_hidden_layer(self, idx):
        # return GraphConv(self.h_dim * self.num_heads, self.h_dim * self.num_heads, norm='right', activation=F.relu)
        return GATConv(self.h_dim * self.num_heads, self.h_dim, self.num_heads, feat_drop=self.dropout, activation=F.relu, mcdropedge=self.mcdropedge)

    def build_output_layer(self):
        return GATConv(self.h_dim * self.num_heads, self.out_dim, self.num_heads, feat_drop=self.dropout, mcdropedge=self.mcdropedge)
        """
        if self.simplify:
...
def main(args):
    ...
    for epoch in range(args.epoch):
        ...
        loss = train_loss #LOSS(logits[train_idx], labels[train_idx])
        t1 = time.time()
        loss.backward()
        optimizer.step()
        t2 = time.time()
        ...
        print(f'epoch {epoch}, val_loss:{val_loss.item()}, train loss:{loss.item()}')
    ...
    torch.cuda.empty_cache() 
    np.savez('loss.npz', train_loss=np.asarray(train_losses), test_micro=np.asarray(test_micros), val_loss=np.asarray(valid_losses))
...
def opt_objective(trial, args):
    ...
    args.num_heads = trial.suggest_int('num_heads', 0, 20)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mcdropedge", type=float, default=0.5,
                        help="dropout probability")
    ...
    if args.search:
        search_space = {'hidden_dim':[8, 16, 32, 64], 'num_heads':[8], 'num_layers':[2,3,4],'dropout': [0.3, 0.5, 0.7]}
