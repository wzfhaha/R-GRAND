...
class EntityClassify(BaseGAT):
    ...
    def build_hidden_layer(self, idx):
        return GATConv(self.h_dim * self.num_heads, self.h_dim, self.num_heads, feat_drop=self.dropout, attn_drop=self.dropout, activation=F.relu)

    def build_output_layer(self):
        return GATConv(self.h_dim * self.num_heads, self.out_dim, self.num_heads, feat_drop=self.dropout, attn_drop=self.dropout)
        """
        if self.simplify:
...
if __name__ == '__main__':
    ...
    if args.search:
        search_space = {'hidden_dim':[8, 16, 32, 64], 'num_heads':[8], 'num_layers':[2,3,4,5],'dropout': [0.3, 0.5, 0.7]}
