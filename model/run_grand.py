...
# from baseline_semi.run_rgrand_hgt import consis_loss_multi
sys.path.append('../')
import time
import argparse
...
from utils.data import load_data, load_data_semi
#from utils.tools import index_generator, evaluate_results_nc, parse_minibatch
from GNN import RGRAND, GRAND
import dgl
import optuna
import joblib
import ipdb
...
def main(args):
    ...
    features_list, adjM, labels, train_val_test_idx,  links, num_nodes,_,_ = load_data_semi(args.dataset, args.run, args.path, train_ratio=args.train_ratio, valid_ratio=args.valid_ratio)
    ...
    node2type = {}
    s = 0
    for t, e in enumerate(num_nodes):
    ...
    for u, v in zip(*g.edges()):
        ...
        """
        if u == v:
            e_feat.append(len(links)+node2type[u])
        else:
        """
    ...
    for _ in range(args.repeat):
        ...
        #net = RGRAND(g, len(links) + 1, in_dims, args.hidden_dim, num_classes, args.num_layers, heads, F.relu, args.dropout, args.mcdropedge, args.slope, feats_type, args.pre_alpha, edge2type, args.layer_norm, sum(num_nodes), fix_edge=args.fix_edge, wo_norm=args.wo_norm, drop_blocks=args.drop_blocks)
        net = GRAND(g, in_dims, args.hidden_dim, num_classes, args.num_layers,  F.elu, args.layer_norm, args.dropout, args.num_mlp)
        net.to(device)
        optimizer = torch.optim.Adam(net.parameters(), lr=args.lr, weight_decay=args.wd)
        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='checkpoint/checkpoint_{}_{}_{}_{}_org.pt'.format(args.dataset, args.num_layers, args.lam, args.train_ratio))
        for epoch in range(args.epoch):
            ...
            valid_result = evaluate_valid(val_pred, val_y_true)
        ...
        net.load_state_dict(torch.load('checkpoint/checkpoint_{}_{}_{}_{}_org.pt'.format(args.dataset, args.num_layers, args.lam, args.train_ratio)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            t1 = time.time()
            ...
            if not multi_label:
                ...
                test_y_true = labels[test_idx].cpu().numpy()
                val_pred = (val_logits.cpu().numpy() > 0.0)
                val_y_true = labels[val_idx].cpu().numpy()
...
def opt_objective(trial, args):
    ...
    args.edge_alpha = trial.suggest_float('edge_alpha', 0.0, 1.0)
    args.alpha = trial.suggest_float('alpha', 0.0, 1.0)
    """
    args.num_mlp = trial.suggest_int('num-mlp', 0, 1000)
    args.num_layers = trial.suggest_int('num-layers', 0, 1000)
    args.hidden_dim = trial.suggest_int('hidden-dim', 0, 1000)
    args.lam = trial.suggest_float('lam', 0.0, 1.0)
    args.wd = trial.suggest_float('wd', 0.0, 1.0)
...
def run_best(args, study):
    #{'drop_blocks': [8, 4, 2], 'beta':[0.3, 0.0, 0.35], 'lam':[0.5, 0.2, 1.0], 'wd': [1e-4, 5e-5, 1e-5]}
    args.num_mlp = study.best_trial.params['num-mlp']
    args.lam = study.best_trial.params['lam']
    args.num_layers = study.best_trial.params['num-layers']
    args.wd = study.best_trial.params['wd']
    args.hidden_dim = study.best_trial.params['hidden-dim']
    print(args)
    valid_res_list = []
    test_res_list = []
    for i in range(args.run_num):
...
if __name__ == '__main__':
    ...
    ap.add_argument('--run_num', type=int, default=5)
    ...
    ap.add_argument("--conf", type=str, default='prob',
                        help="confidence type")
    ap.add_argument('--path', type=str, default='/data/wenzheng/Heter_data_clean/')
    ap.add_argument('--search', default=False, action='store_true')
    ap.add_argument('--wo_norm', default=False, action='store_true')
    ...
    ap.add_argument('--num-mlp', type=int, default=2)
    ap.add_argument("--aug", type=str, default='dropout')
    ap.add_argument('--stop_mode', type=str, default='loss')

    args = ap.parse_args()
    if args.search:
        search_space = {'num-mlp': [4, 2, 1], 'lam':[0.5, 0.2, 1.0], 'hidden-dim':[64, 128, 256], 'num-layers':[2, 3, 4],'wd': [1e-4, 5e-5, 1e-5, 0.0]}
        study = optuna.create_study(sampler=optuna.samplers.GridSampler(search_space))
        study.optimize(lambda trial: opt_objective(trial, args), n_trials=args.trials)
        joblib.dump(study, f'{args.dataset}_{args.hidden_dim}_{args.num_heads}_{args.num_layers}_study.pkl')
        run_best(args, study)
    else:
        valid_res_list = []
        test_res_list = []
