...
def opt_objective(trial, args):
    ...
    """
    args.drop_blocks = trial.suggest_int('drop_blocks', 0, 1000)
    args.beta = trial.suggest_float('beta', 0.0, 1.0)
...
def run_best(args, study):
    ...
    args.beta = study.best_trial.params['beta']
...
if __name__ == '__main__':
    ...
    ap.add_argument('--drop_blocks', type=int, default=4)
    ap.add_argument("--aug", type=str, default='dropout')
    ap.add_argument('--stop_mode', type=str, default='loss')

    args = ap.parse_args()
    if args.search:
        search_space = {'drop_blocks': [8, 4, 2, 1], 'beta':[0.3, 0.0, 0.35], 'lam':[0.5, 0.2, 1.0], 'wd': [1e-4, 5e-5, 1e-5, 0.0]}
