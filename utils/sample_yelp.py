...
phrase_sample = np.random.choice(num_nodes[-1], 10000, replace=False) + sum(num_nodes[:-1])


#sampled_idx = (np.asarray(adjM[labeled_nodes][:,np.arange(num_nodes[0], sum(num_nodes))].sum(0)).flatten() > 0).nonzero()[0]
sampled_idx = np.concatenate([labeled_nodes, np.arange(num_nodes[0], num_nodes[0] + num_nodes[1]), np.arange(num_nodes[0]+ num_nodes[1], sum(num_nodes[:3])),phrase_sample])
...
sampled_map = {}
with open('../data/Yelp_sample/node.dat', 'w') as f:
...
with open('../data/Yelp_sample/label.dat', 'w') as f:
    for new_idx, (label, idx) in enumerate(zip(label_s, final_sampled_idx)):
        if np.sum(label) == 0:
            continue
        name = nodeidx2name[idx]
        type = nodeidx2type[idx]
        label_numid = label.nonzero()[0]
        f.write(f'{idx}\t{name}\t{type}\t{",".join([str(x) for x in label_numid])}\n')

with open('../data/Yelp_sample/link.dat', 'w') as f:
