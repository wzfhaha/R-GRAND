...
"""
sampled_idx = (np.asarray(adjM[labeled_nodes][:,np.arange(num_nodes[0], sum(num_nodes))].sum(0)).flatten() > 1).nonzero()[0]
sampled_idx = np.concatenate([labeled_nodes, sampled_idx + num_nodes[0]])
adjM_sample = adjM[sampled_idx][:,sampled_idx]
label_sample = label[sampled_idx]
final_idx = (np.asarray(adjM_sample.sum(1)).flatten()>0).nonzero()[0]
...
sampled_map = {}
with open('../data/Freebase_sample/node.dat', 'w') as f:
...
with open('../data/Freebase_sample/label.dat', 'w') as f:
...
with open('../data/Freebase_sample/link.dat', 'w') as f:
